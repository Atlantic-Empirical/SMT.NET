...
Namespace Multimedia.Players.YUV

    Public Class cYUVPlayer
...
#Region "cBasePlayer Overrides"
        ...
        Public Overrides Function BuildGraph(ByVal FilePath As String, ByVal NotifyWindowHandle As Integer, ByVal nAVMode As eAVMode, ByVal nParentForm As Form) As Boolean
            Try
                ...
                HR = Graph.iYUVSource.Load(Replace(FilePath, "\", "\\"), 0)
                If HR < 0 Then Marshal.ThrowExceptionForHR(HR)
                HR = Graph.iYUVSource.SetATPF(Me.TargetATPF)
                If HR < 0 Then Marshal.ThrowExceptionForHR(HR)
                ...
                'Connect pins
                HR = Graph.GraphBuilder.Connect(Graph.YUV_Out, Graph.DLV_In)
                If HR < 0 Then Marshal.ThrowExceptionForHR(HR)
            ...
            Catch ex As Exception
                Throw New Exception("Problem with BuildVideoGraph YUV Frame Splitter. Error: " & ex.Message)
                Return False
            End Try
        End Function
        ...
        Public Overrides Function FilterCheck() As String
            Try
                ...
                If Not SMT_YUVSource Then msgstr &= "SMT YUV Source" & vbNewLine
        ...
        Public Overrides Function SetPlayerProperty(ByVal PropertyID As Integer, ByVal PropertyValue As Long, ByVal PropetyString As String) As Boolean
            Try
                Select Case PropertyID
                    Case 0 'TargetATPF
                        Me.TargetATPF = PropertyValue
                        Return True
                    Case Else
                        Return False
                End Select
            Catch ex As Exception
                Throw New Exception("Problem with SetPlayerProperty(). Error: " & ex.Message, ex)
            End Try
        End Function
...
#Region "Shuttle"
        ...
        Private Sub StartShuttle(ByVal Speed As eShuttleSpeed, ByVal Forward As Boolean)
            ...
            'Might need to add similar functionality to the YUV source filter
            'KO_IKeystone.ActivateFFRW()
