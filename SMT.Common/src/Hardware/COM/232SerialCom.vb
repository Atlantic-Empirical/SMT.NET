...
Namespace Hardware.COM.SerialPort
            ...
            If miPort > 0 Then
                Try
                    ...
                    Else
                        ...
                        'Dim sErrTxt As String = New Win32Exception().Message 'changed 10/19/06 TRPF
                        Dim sErrTxt As String = "No serial port (RS232) port found on this system."
                        Throw New CIOChannelException("Unable to open COM port " + miPort.ToString + ControlChars.CrLf + sErrTxt)
                    End If
                        ...
                        If (lMask And EventMasks.RxChar) > 0 Then
                            ...
                            If iErrMask = 0 Then
                                ...
                                If ReadFile(mhRS, mabtRxBuf, uComStat.cbInQue, iBytesRead, ovl) > 0 Then
                                    ''DEBUGGING
                                    'Debug.Write("Bytes in pEventsWatcher() = ")
                                    'For Each b As Byte In mabtRxBuf
                                    '    Debug.Write(Hex(b) & " ")
                                    'Next
                                    'Debug.WriteLine("")
                                    ''DEBUGGING         
